...
def main(args):
    ...
    corr_files = list(sorted(f for f in os.listdir(args.correct_dir) if f.endswith('.json')))
    pred_files = list(sorted(f for f in os.listdir(args.predict_dir) if f.endswith('.json')))
    file_cnt = len(corr_files)
