...
from nnet.grad import GradientReversalLayer
...
class BertPreTrainedModel(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_path, config_path, *inputs, **kwargs):
        """
        Instantiate a BertPreTrainedModel from a pre-trained model file or a pytorch state dict.
        Download and cache the pre-trained model file if needed.

        Params:
            pretrained_model_path: a path or url to a pretrained model 
            config_path: a configuration file for the model
        ...
        logger.info("loading weights file {}".format(pretrained_model_path))
...
class BertForTaskNLU(BertPreTrainedModel):
    """BERT model for Task-Orient NLU ."""
    def __init__(self, config, output_attentions=False, 
            keep_multihead_output=False, *inputs, **kwargs):
        ...
        self.domain_num = kwargs["domain_num"]
        self.intent_num = kwargs["intent_num"]
        ...
        self.slots_outputs = nn.Linear(config.hidden_size, self.label_num)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, head_mask=None):
        outputs = self.bert(input_ids, token_type_ids, attention_mask,
                                                       output_all_encoded_layers=False,
                                                       head_mask=head_mask)
        sequence_output, cls_output = outputs
