...
class BinarySLUTagger(nn.Module):
    ...
    def crf_loss(self, inputs, lengths, y):
        ...
        Ouput:
            crf_loss: loss of crf
        """
        padded_y = self.pad_label(inputs, y)
        crf_loss = self.crf_layer.loss(inputs, padded_y)

        return crf_loss

    def pad_label(self, inputs, y):
        bsz = inputs.size(0)
        max_len = inputs.size(1)
        padded_y = torch.LongTensor(bsz, max_len).fill_(SLOT_PAD)
        for i in range(bsz):
            y_i = y[i]
            padded_y[i, 0:len(y_i)] = torch.LongTensor(y_i)
...
class IntentPredictor(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.self_attn = Attention(attention_size=self.input_dim)

        self.intent_set = Vocab.from_file(os.path.join(params.data_path, "intents.txt"))
        self.intent_pred = nn.Linear(self.input_dim, self.intent_set.n_words)

        self.domain_set = Vocab.from_file(os.path.join(params.data_path, "domains.txt"))
        with open(os.path.join(params.data_path, "dom2intents.json"), 'r', encoding='utf8') as fd:
            self.domain2intents = json.load(fd)
      
    def forward(self, hidden_layers, lengths):
        """
            domains: T(B)
            hidden_layers: T(BSD)
            lengths: T(B)
        """
        sent_repr, _ = self.self_attn(hidden_layers, lengths)
        logits_int = self.intent_pred(sent_repr)
        return logits_int

    def predict(self, domains, hidden_layers, lengths):
        """
            domains: L(B:int)
        """
        pred_ints = []
        logits_int = self.forward(hidden_layers, lengths)
        for dom, lg in zip(domains, logits_int):
            dom_name = self.domain_set[dom]
            intent_cands = set(self.domain2intents[dom_name])
            mask = torch.ByteTensor([0 if i in intent_cands else 1 for i in self.intent_set._vocab]).cuda()
            lg = lg.masked_fill(mask=mask, value=-np.inf)
            pred_int = torch.argmax(lg)
            pred_ints.append(self.intent_set.index2word[pred_int.item()])
        return pred_ints
