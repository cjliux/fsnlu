    ...
    def train_step(self, batch, epoch=None):
        ...
        for i_sam in range(batch_size):
            ...
            if self.use_label_encoder:
                ...
                sup_loss += template0_loss + template1_loss + template2_loss
                ...
                if epoch > 3:
                    ...
                    sup_loss += input_loss0 + input_loss1 + input_loss2
...
def train_model(params):
    ...
    slotname_predictor = SlotNamePredictor(params, vocab, binary_slutagger.lstm.embedding)
...
def eval_model(params):
    ...
    slotname_predictor = SlotNamePredictor(params, vocab, binary_slu_tagger.lstm.embedding)
...
def test_model(params):
    ...
    #     params.batch_size, params.tr, params.n_samples)
    dataloaders_test = get_dataloader_for_fs_test(
        params.data_path, params.raw_data_path, 
        params.batch_size, params.tr, params.n_samples, 
        sep_dom=True)
    ...
    for dom, dataloader_test in dataloaders_test.items():
        print("Domain {}".format(dom))
        final_items = slu_trainer.predict(dataloader_test)
        pred_items = list(final_items.values())[0]
        with open(os.path.join(save_dir, "predict_{}.json".format(dom)), 'w', encoding='utf8') as fd:
            json.dump(pred_items, fd, ensure_ascii=False, indent=2)
