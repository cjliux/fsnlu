...
from .data import (get_dataloader, get_dataloader_for_train, 
    get_dataloader_for_support, get_dataloader_for_test)
...
def do_comb_train_no_eval(args):
    ...
    ## def data
    train_loader = get_dataloader_for_train(args, tokenizer)
    sup_loaders = get_dataloader_for_support(args, tokenizer, sep_dom=True)

    test_doms = (args.tst_dm.split(',') 
        if args.tst_dm.strip() != 'all' else sup_loaders.keys())

    for dom in sorted(sup_loaders.keys()):
        if dom not in test_doms:
            continue

        logger.info("[Train] train model for test domain {}".format(dom))
        # test_loader = test_loaders[dom]
        test_suploader = sup_loaders[dom]
...
def do_predict(args):
    ...
    test_loaders = get_dataloader_for_test(args, tokenizer, sep_dom=True)

    test_doms = (args.tst_dm.split(',') 
        if args.tst_dm.strip() != 'all' else test_loaders.keys())

    for dom in sorted(test_loaders.keys()):
        if dom not in test_doms:
            continue
        ...
        state_dict = torch.load(model_path.format(dom), map_location=device)
