...
from .data import get_dataloader, get_dataloader_for_test_only
...
class SLUTrainer(object):
    ...
    def predict(self, dataloader):
        ...
        pbar = tqdm.tqdm(enumerate(dataloader), total=len(dataloader))
        for i, (X, lengths, y_dm, raw_domains, raw_text, ids) in pbar:
            X, lengths = X.cuda(), lengths.cuda()
            bin_preds_batch, lstm_hiddens = self.binary_slu_tagger(X)
            bin_preds_batch = self.binary_slu_tagger.crf_decode(bin_preds_batch, lengths)
            ...
            for i_sam, (x, l_x, fp) in enumerate(zip(X, lengths, final_preds_batch)):
                ...
                for etype, start, end in ents:
                    val = raw_text[i_sam][start:end+1]
                ...
                item = {}
                item['domain'] = raw_domains[i_sam]
                item['text'] = raw_text[i_sam]
                item['id'] = ids[i_sam]
                item['slots'] = slvals
...
def train_model(params):
    ...
    dataloader_tr, dataloader_val, dataloader_test = get_dataloader(
        params.data_path, params.evl_dm.split(','), params.tgt_dm.split(','), 
...
def test_model(params):
    dev_qry_data, dataloader_test = get_dataloader_for_test_only(
        params.data_path, params.tgt_dm.split(','), 
        params.batch_size, params.tr, params.n_samples)
